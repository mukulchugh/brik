/**
 * Shared Demo App Component for Brik Examples
 * Used by both Expo and React Native CLI example apps
 */
...
export interface BrikDemoAppProps {
  /**
   * App title (e.g., "Brik Expo App" or "Brik Test App")
   */
  title: string;
   ...
   */
  showHotReloadStatus?: boolean;
}

export function BrikDemoApp({
  title,
  enableHotReload = __DEV__,
  showHotReloadStatus = true,
}: BrikDemoAppProps): React.JSX.Element {
  const [activityId, setActivityId] = useState<string | null>(null);
  const [status, setStatus] = useState<string>('Ready');

  // Enable hot reload for widgets in development
  if (enableHotReload && typeof useBrikHotReload !== 'undefined') {
    useBrikHotReload({
...
- P99: ${stats.p99}ms
  ...
  const checkStorage = async () => {
    const lastActivity = await widgetStorage.load<{id: string; startedAt: number}>('lastActivity');
    if (lastActivity) {
      Alert.alert(
        'Last Activity',
        `ID: ${lastActivity.id}\nStarted: ${new Date(
      ...
      <ScrollView contentInsetAdjustmentBehavior="automatic">
        ...
        {showHotReloadStatus && (
          <View style={styles.section}>
            ...
            <Text style={styles.hotReloadText}>
              {enableHotReload ? '🔥 Hot reload enabled' : '❄️ Production mode'}
            </Text>
            {enableHotReload && (
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            Testing: Error Handling, Performance Monitoring, Widget Storage
            {enableHotReload && ', Hot Reload'}
          </Text>
        </View>
      </ScrollView>
    </SafeAreaView>
