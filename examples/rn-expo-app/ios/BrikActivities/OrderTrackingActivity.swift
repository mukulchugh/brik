        ...
        ActivityConfiguration(for: OrderTrackingAttributes.self) { context in
            ...
            DynamicIsland {
                    ...
                    VStack(alignment: .leading, spacing: 0) {
                      HStack(alignment: .center, spacing: 12) {
                        ...
                        VStack(alignment: .center, spacing: 0) {
                          ...
                          Text("Acme Pizza").font(.system(size: 12)).foregroundStyle(Color(.sRGB, red: 0.420, green: 0.447, blue: 0.502, opacity: 1.000))
                        }
                      }
                      ProgressView(value: 0.65).frame(height: 6).cornerRadius(3)
                      HStack(alignment: .center, spacing: 8) {
                        Text("Delivering").font(.system(size: 12)).fontWeight(.medium)
                ...
                HStack(alignment: .center, spacing: 4) {
                  ...
                  ProgressView(value: 0.65).frame(width: 30, height: 4)
                }
            } compactTrailing: {
                Text("")
            } minimal: {
...
class OrderTrackingHandler: BrikActivityHandler {
    ...
    func startActivity(staticAttributes: [String: Any], dynamicAttributes: [String: Any]) throws -> String {
        ...
        guard let merchantName = staticAttributes["merchantName"] as? String else {
            throw BrikActivityError.invalidAttributes("Missing or invalid 'merchantName' in static attributes")
        }
        guard let itemCount = staticAttributes["itemCount"] as? Double else {
            throw BrikActivityError.invalidAttributes("Missing or invalid 'itemCount' in static attributes")
        }
        ...
        guard let progress = dynamicAttributes["progress"] as? Double else {
            throw BrikActivityError.invalidAttributes("Missing or invalid 'progress' in dynamic attributes")
        }

        // Create attributes
        let attrs = OrderTrackingAttributes(orderId: orderId, merchantName: merchantName, itemCount: itemCount)
